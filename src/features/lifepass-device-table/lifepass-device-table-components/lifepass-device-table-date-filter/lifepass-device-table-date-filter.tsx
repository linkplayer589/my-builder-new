...
import {
  LIFEPASS_DEVICE_TABLE_DATE_FILTER_ID,
  LIFEPASS_DEVICE_TABLE_DATE_FROM_ROW_ID,
  LIFEPASS_DEVICE_TABLE_DATE_PLACEHOLDER,
  LIFEPASS_DEVICE_TABLE_DATE_TO_ROW_ID,
} from "./lifepass-device-table-date-filter-constants"
...
interface LifepassDeviceTableDateFilterProps {
  initialDateFrom?: string
  initialDateTo?: string
}

/**
 * Lifepass device table date filter
 *
 * @description
 * Keeps the date picker selection synchronized with the URL `filters`
 * parameter so the underlying table fetches the correct date range directly
 * from the database.
 */
export function LifepassDeviceTableDateFilter({
  initialDateFrom,
  initialDateTo,
}: LifepassDeviceTableDateFilterProps) {
  ...
  const handleUpdate = React.useCallback(
    ({ range }: { range?: { from?: Date; to?: Date } }) => {
      ...
      const nextFilters = [
        ...filters.filter(
          (filter) =>
            filter.rowId !== LIFEPASS_DEVICE_TABLE_DATE_FROM_ROW_ID &&
            filter.rowId !== LIFEPASS_DEVICE_TABLE_DATE_TO_ROW_ID
        ),
        buildFilter(LIFEPASS_DEVICE_TABLE_DATE_FROM_ROW_ID, "gte", fromISO),
        buildFilter(LIFEPASS_DEVICE_TABLE_DATE_TO_ROW_ID, "lte", toISO),
      ]
  ...
  const handleClear = React.useCallback(() => {
    ...
    setLabel(LIFEPASS_DEVICE_TABLE_DATE_PLACEHOLDER)
...
function formatDisplayLabel(from?: string, to?: string): string {
  if (!from || !to) {
    return LIFEPASS_DEVICE_TABLE_DATE_PLACEHOLDER
  }
...
function buildFilter(
  ...
  return {
    id: LIFEPASS_DEVICE_TABLE_DATE_FILTER_ID,
    rowId,
    operator,
    value,
    type: "date",
