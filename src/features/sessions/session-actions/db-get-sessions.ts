...
type SessionFilter = {
    ...
    operator: "eq" | "iLike" | "notILike" | "lt" | "lte" | "gt" | "gte" | "neq" | "contains" | "notContains" | "between" | "empty" | "notEmpty";
    rowId: string;
    ...
    try {
        ...
        if (input.sort.length > 0) {
            try {
                const validSorts = input.sort
                    ...
                    .map((item) => {
                        const columnName = item.id.replace('sessions_', '') as keyof typeof sessions
                        const column = sessions[columnName] as AnyColumn
                        return item.desc ? desc(column) : asc(column)
                    })

                orderBy = validSorts.length > 0 ? validSorts : [desc(sessions.createdAt)]
            } catch (error) {
        ...
        const { data, total } = await db.transaction(async (tx) => {
                ...
                for (const order of ordersWithSessions) {
                    if (order.sessionIds) {
                        for (const sessionId of order.sessionIds) {
                            if (sessionIds.includes(sessionId) && !orderIdMap.has(sessionId)) {
            ...
            const dataWithOrderIds = data.map(session => {
                ...
                const orderId = orderIdFromLog ?? orderIdMap.get(session.sessions.id) ?? null
