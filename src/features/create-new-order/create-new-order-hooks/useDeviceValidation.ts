...
} from "../create-new-order-actions/check-device-status/types"
...
export function useDeviceValidation(deviceId: string) {
    ...
    const validateDevice = useCallback(async () => {
        ...
        try {
            ...
            if (!result.success) {
                setError(result.error instanceof Error ? result.error.message : "Failed to validate device")
                setValidationResult(null)
                return
            }

            const validation = validateDeviceStatus(result.data)
