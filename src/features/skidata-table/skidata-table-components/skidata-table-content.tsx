...
import { SkiDataTable } from "../skidata-table"

interface SkidataTableContentProps {
  ...
  /** Total owed for live items */
  totalOwedLive: number
}
 ...
 */
export function SkidataTableContent({
  items,
  resortId,
  liveItems,
  testItems,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-lg border p-4">
          <div className="text-sm text-muted-foreground">Live Tickets</div>
          <div className="text-2xl font-bold">{liveItems}</div>
        </div>
        <div className="rounded-lg border p-4">
          <div className="text-sm text-muted-foreground">Total Owed (Live)</div>
          <div className="text-2xl font-bold">€{totalOwedLive.toFixed(2)}</div>
        </div>
      </div>

      {/* Table with filtered data */}
      <SkiDataTable data={filteredItems} resortId={resortId} />
    </div>
  )
}
