...
export interface DateRangePickerProps {
  ...
  showCompare?: boolean
  /** Option for showing time selection */
  showTime?: boolean
  /** Optional custom label to render inside the trigger button */
  displayValue?: string
}
 ...
 */
const formatDate = (date: Date, locale: string = "en-us", showTime: boolean = false): string => {
...
export const DateRangePicker: FC<DateRangePickerProps> & {
  ...
  useEffect(() => {
    ...
    const cloneRange = (value?: DateRange): DateRange | undefined => {
    ...
    useEffect(() => {
      if (isOpen) {
        openedRangeRef.current = cloneRange(range)
        openedRangeCompareRef.current = cloneRange(rangeCompare)
      }
    }, [isOpen])
            ...
            <div className="text-right">
              <div className="py-1">
                <div suppressHydrationWarning>
                  {displayValue ??
                    `${formatDate(range.from, locale, showTime)}${
                      range.to != null
                        ? " - " + formatDate(range.to, locale, showTime)
                        : ""
                    }`}
                </div>
              </div>
              {rangeCompare != null && (
                <div className="-mt-1 text-xs opacity-60">
                  <>
                    vs. {formatDate(rangeCompare.from, locale, showTime)}
                    {rangeCompare.to != null
                      ? ` - ${formatDate(rangeCompare.to, locale, showTime)}`
                      : ""}
                  </>
                </div>
              )}
                ...
                <div className="flex flex-col items-center justify-end gap-2 px-3 pb-4 lg:flex-row lg:items-start lg:pb-0">
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-end gap-2">
                      <div className="flex flex-col gap-1">
                        {showTime && <Label className="text-xs text-muted-foreground">From</Label>}
                        <DateInput
                          value={range.from}
                          onChange={(date) => {
                            // Preserve the time from the current range.from
                      ...
                      <div className="flex flex-col gap-1">
                        {showTime && <Label className="text-xs text-muted-foreground">To</Label>}
                        <DateInput
                          value={range.to}
                          onChange={(date) => {
                            // Preserve the time from the current range.to
                            const newDate = new Date(date)
                            if (range.to) {
                              newDate.setHours(
                                range.to.getHours(),
                ...
                <div>
                  <Calendar
                    ...
                    onSelect={(value: { from?: Date; to?: Date } | undefined) => {
                      if (value?.from != null) {
                        // Preserve times when selecting dates from calendar
