...
import { getDefaultFilterOperator } from "@/components/data-table"
import { getFilterOperators } from "@/components/data-table"
...
export function DataTableFilterList<TData>({
   ...
   * For advanced filtering, we skip data structure validation since filter IDs 
   * may reference database columns not present in joined data.
   */
  const filtersParser = React.useMemo(() => {
    const firstRow = table.getRowModel().rows[0]?.original
    const hasComplexStructure = firstRow && typeof firstRow === 'object' && 
      Object.values(firstRow).some(val => val && typeof val === 'object' && !Array.isArray(val))
    
    // Return parser without validation for complex structures to avoid type errors
    return hasComplexStructure 
      ? getFiltersStateParser<TData>()
      : getFiltersStateParser<TData>(firstRow)
  }, [table])

  const [filters, setFilters] = useQueryState(
    "filters",
    filtersParser
      .withDefault([] as Filter<TData>[])
      .withOptions({
        clearOnDefault: true,
        shallow,
      })
  ...
  /**
   * Ensure filters is always an array for type safety.
   * Memoized to prevent unnecessary re-renders in useEffect dependencies.
   */
  const safeFilters: Filter<TData>[] = React.useMemo(() => {
    return Array.isArray(filters) ? filters : []
  }, [filters])

  /**
   * Auto-update table data when filters change.
   * This effect triggers data fetching whenever the user modifies filters,
   * ensuring the table results reflect the new filter criteria.
   */
  React.useEffect(() => {
    // Only trigger updates if there are active filters
    if (safeFilters.length > 0) {
      debouncedOnUpdate()
    }
  }, [safeFilters, debouncedOnUpdate])
  ...
  function updateFilter({
    ...
    updateFunction((prevFilters: Filter<TData>[]) => {
  ...
  function removeFilter(rowId: string) {
    const updatedFilters = safeFilters.filter((filter) => filter.rowId !== rowId)
    void setFilters(updatedFilters)
  }

  function moveFilter(activeIndex: number, overIndex: number) {
    void setFilters((prevFilters: Filter<TData>[]) => {
      const currentFilters = Array.isArray(prevFilters) ? prevFilters : []
      const newFilters = [...currentFilters]
      const [removed] = newFilters.splice(activeIndex, 1)
      if (!removed) return currentFilters
    ...
    if (filter.operator === "empty" || filter.operator === "notEmpty") {
      return (
        <div
          ...
          role="status"
          aria-live="polite"
          aria-label={`${filterField.label} filter is ${filter.operator === "empty" ? "empty" : "not empty"}`}
  ...
  return (
    <Sortable
      value={safeFilters.map((item) => ({ id: item.rowId }))}
      ...
      <Popover>
        ...
        <PopoverContent
          ...
          className={cn(
            ...
            safeFilters.length > 0 ? "gap-3.5" : "gap-2"
          )}
        >
          {safeFilters.length > 0 ? (
            ...
            {safeFilters.map((filter, index) => {
              ...
              return (
                <SortableItem key={filter.rowId} value={filter.rowId} asChild>
                  <div
                    className={`flex min-w-0 ${isMobile ? "flex-col" : "items-center"} gap-2`}
                  >
                    <div
                      className={`min-w-[4.5rem] ${!isMobile ? "text-center" : "text-left"}`}
                    >
                      {index === 0 ? (
                    ...
                    <Popover modal>
                      <PopoverTrigger asChild>
                        <Button
                          ...
                          className={`h-8 ${!isMobile ? "w-32" : ""} justify-between gap-2 rounded focus:outline-none focus:ring-1 focus:ring-ring focus-visible:ring-0`}
                        >
                    ...
                    >
                      <SelectTrigger
                        ...
                        className={`h-8 ${!isMobile ? "w-32" : ""} rounded`}
                      >
          ...
          <div
            className={`flex w-full gap-2 ${isMobile ? "flex-col" : "items-center"}`}
          >
            <Button
              size="sm"
              ...
              <>
                ...
                {onUpdate && (
                  <Button
                    size="sm"
                    variant="default"
                    className={`${!isMobile ? "ml-auto" : ""} rounded`}
