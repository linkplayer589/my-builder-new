...
export function DataTableSortList<TData>({
  ...
  /**
   * Auto-update table data when sorting changes.
   * This effect triggers data fetching whenever the user modifies sorting,
   * ensuring the table results reflect the new sort order.
   */
  React.useEffect(() => {
    // Skip initial load, only trigger on actual user changes
    const hasUserSorting = uniqueSorting.length > 0
    const hasResetToDefault = sorting.length === 0 && initialSorting.length > 0
    
    if (hasUserSorting || hasResetToDefault) {
      debouncedOnUpdate()
    }
  }, [uniqueSorting, debouncedOnUpdate, sorting.length, initialSorting.length])
    ...
    >
      <Popover>
        ...
        >
                    ...
                    >
                      <Popover modal>
                        <PopoverTrigger asChild>
                          <Button
                            ...
                            size="sm"
                            role="combobox"
                            className={`h-8 ${!isMobile ? "w-44" : ""} justify-between gap-2 rounded focus:outline-none focus:ring-1 focus:ring-ring`}
                      ...
                      >
                        <SelectTrigger
                          ...
                          className={`h-8 ${!isMobile ? "w-24" : ""}rounded`}
                        >
          ...
          <div
            className={`flex w-full ${isMobile ? "flex-col" : "items-center"} gap-2`}
          >
            <Button
              size="sm"
              ...
              <>
                ...
                {onUpdate && (
                  <Button
                    size="sm"
                    variant="default"
                    className={`rounded ${!isMobile ? "ml-auto" : ""}`}
