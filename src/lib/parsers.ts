import type { ExtendedSortingState, Filter } from "@/types/index"
...
export const getSortingStateParser = <TData>(
  ...
  return createParser<ExtendedSortingState<TData>>({
    parse: (value) => {
      try {
        const parsed = JSON.parse(value) as ExtendedSortingState<TData>
...
export const getFiltersStateParser = <T>(originalRow?: Row<T>["original"]) => {
  ...
  return createParser<Filter<T>[]>({
    parse: (value) => {
      try {
        const parsed = JSON.parse(value) as Filter<T>[]
